...
public class KStreamFailableForeach<K, V> implements ProcessorSupplier<K, V> {
    private static final Logger LOG = LoggerFactory.getLogger(KStreamFailableForeach.class);

    private static final Long ATTEMPTS_PUNCTUATE_INTERVAL_MS = 500L;
    private final FailableForeachAction<? super K, ? super V> action;
    private final String tasksStoreName;
    private final String deadLetterNodeName;
    private final ObjectMapper objectMapper = new ObjectMapper();

    public KStreamFailableForeach(String tasksStoreName, String deadLetterNodeName, final FailableForeachAction<? super K, ? super V> action){
    ...
    @Override
    public Processor<K, V> get() { return new FailableKStreamRetryableForeachProcessor(); }

    private class FailableKStreamRetryableForeachProcessor extends AbstractProcessor<K, V> {
        ...
        private void logRetryableException(TaskAttempt attempt, K key, FailableKStream.RetryableException e){
            LOG.warn(
        ...
        private void logFailableException(TaskAttempt attempt, K key, FailableKStream.FailableException e){
            LOG.error(
