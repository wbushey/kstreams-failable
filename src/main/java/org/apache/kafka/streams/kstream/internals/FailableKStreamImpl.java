...
public class FailableKStreamImpl<K, V> extends KStreamImpl<K, V> implements FailableKStream<K, V> {
     ...
     */
    FailableKStreamImpl(String name, Serde<K> keySerde, Serde<V> valueSerde, Set<String> sourceNodes, boolean repartitionRequired, StreamsGraphNode streamsGraphNode, InternalStreamsBuilder builder) {
        super(name, keySerde, valueSerde, sourceNodes, repartitionRequired, streamsGraphNode, builder);
    }

    /*
     * Constructor that decorates a {@code KStreamImpl} with failure handling methods
     */
    public FailableKStreamImpl(KStreamImpl<K, V> stream) {
    ...
    @Override
    public void retryableForeach(final FailableForeachAction<? super K, ? super V> action) {
        retryableForeach(action, builder.newProcessorName(NamedInternal.empty().name()));
    }

    @Override
    public void retryableForeach(final FailableForeachAction<? super K, ? super V> action, String name) {
        ...
        final ProcessorParameters<? super K, ? super V> processorParameters = new ProcessorParameters<>(
                new KStreamFailableForeach<>(retries_store_name, deadLetterPublisherNodeName, action),
                nodeName
        );
